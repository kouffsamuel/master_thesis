...
import numpy as np
from processing.radar_processing import compute_rd
...
background_puissance = np.load(f"/home/skouff/background_power_with_plexi.npy")
...
def main(rd=True, camera=True, lidar=False):
    ...
    with writer.saving(viewer.fig, output_video, dpi=200):
        for i in range(1, len(raw_files)):
            ...
            # background_power_hamming.append(rd_power)
            ...
            if lidar:
                ...
                viewer.update(times=raw_times[i], camera_times=cam_times[i], rd_power=rd_power, img=img, pts=pts)
    ...
    # background_power = np.mean(background_power_hamming, axis=0)
    # np.save(f"/Benson_DATA3/Public/MUSE/background_power_with_plexi_hamming.npy", background_power)
    # print("/Benson_DATA3/Public/MUSE/background_power_with_plexi_hamming.npy")
