...
class Range_Fourier_Net(nn.Module):
    def __init__(self):
        ...
        # Complex linear layer with fixed Range Fourier weights
        self.range_nn = CplxLinear(512, 512, bias = False)
        range_weights = np.zeros((512, 512), dtype = np.complex64)
        for j in range(0, 512):
            for h in range(0, 512):
                range_weights[h][j] = np.exp(-1j * 2 * np.pi *(j*h/512)) # Fourier transform formula 
...
class Doppler_Fourier_Net(nn.Module):
    def __init__(self):
        super(Doppler_Fourier_Net, self).__init__()
         # Complex linear layer with fixed Doppler Fourier weights
...
# Activation function for complex-valued inputs, can be applied either on the modulus or separately on real and imaginary parts
...
class FFT_Net(nn.Module):
    ...
    def forward(self,x):
        x = x.permute(0,1,3,2)
        x = self.range_net(x)
        x = self.cplx_transpose(2,3)(x) # Transpose to apply Doppler Fourier along the correct dimension
