...
class ADCEmbed(nn.Module):
    def __init__(self, n_samples=512, n_rx=16, embed_dim=96, norm_layer=None):
        super(ADCEmbed, self).__init__()
        self.n_samples = n_samples
        self.n_rx = n_rx
        self.embed_dim = embed_dim

        self.proj = nn.Linear(n_rx * n_samples, embed_dim)
    ...
    def forward(self,x):
        B, n_rx, n_samples, n_chirps = x.size()
        
        x = x.permute(0, 3, 1, 2).contiguous()
        x = x.view(B, n_chirps, -1)
...
class MViT_ADC(nn.Module):
    def __init__(self, D, p, H, W, neuron, mha, layer, dropout, n_encoders=1):
        ...
        self.adc_embed = ADCEmbed(512, 32, self.D)
