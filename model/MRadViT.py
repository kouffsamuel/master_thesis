...
class Detection_Header(nn.Module):
    ...
    def __init__(self, use_bn=True,reg_layer=2,input_angle_size=0, num_classes=4):
...
class MRadViT(nn.Module):
    def __init__(self, D, p, H, W, neuron, mha, layer, dropout, n_encoders=1, data_mode='Custom_RD'):
        ...
        self.patch_embed = PatchEmbed(patch_size=(self.p, self.p), in_chans=2, embed_dim=self.D)
        ...
        self.encoder_layers = nn.ModuleList([nn.ModuleList([
            ...
            for _ in range(self.layer)])] * self.n_encoders)
    ...
    def forward(self, x, x_mask=None):
        ...
        for i in range(T//2):
            re = x[:, i, :, :]
            im = x[:, i + T//2, :, :]   
            xi = torch.cat([re, im], dim=-1)           # (B, H, W*2)
            xi = self.patch_embed(xi)  # (B, D, Hp, W)
            xi = self.pe(xi)  # (B, D, Hp, W)
