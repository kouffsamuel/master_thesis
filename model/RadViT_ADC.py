...
class ADCTokenEmbed(nn.Module):
    def __init__(self, n_antennas, embed_dim):
        super(ADCTokenEmbed, self).__init__()
        self.n_antennas = n_antennas
        self.embed_dim = embed_dim
        self.proj = nn.Conv2d(2 * n_antennas, embed_dim, kernel_size=1, bias=False) 

    def forward(self, x):
        # x: (B, 16, H, W)
        B, A, S, C, _ = x.shape 
        x = x.permute(0, 1, 4, 2, 3).contiguous().view(B, 2 * A, S, C)
        tokens = self.proj(x)              
        tokens = tokens.flatten(2).transpose(1, 2)
        return tokens


class RadViT_ADC(nn.Module):
    def __init__(self, D, p, H, W, neuron, mha, layer, dropout, n_encoders=1):
        ...
        # self.token_embed = ADCTokenEmbed(16, self.D)  
        # Positional encodings séparés
        self.pe = PositionalEncoding(d_model=self.D, max_len=140000)  # nn.Parameter(torch.randn(1, self.Np, self.D))
