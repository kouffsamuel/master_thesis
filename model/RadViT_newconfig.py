...
class RangeAngle_Decoder(nn.Module):
    def __init__(self, D=256):
        ...
        self.deconv4 = nn.ConvTranspose2d( 8,  8, kernel_size=3, stride=(2,1), padding=1, output_padding=(1,0))  
        self.deconv3 = nn.ConvTranspose2d( 32,  32, kernel_size=3, stride=(2,1), padding=1, output_padding=(1,0))  
        self.deconv2 = nn.ConvTranspose2d(64, 64, kernel_size=3, stride=(2,1), padding=1, output_padding=(1,0))  

        self.conv_block4 = BasicBlock(16,  32)   # 16+16=32
        self.conv_block3 = BasicBlock(40, 64)   # 64+16=80
        self.conv_block2 = BasicBlock(72, 256)   # 128+16=144

    def forward(self, features):
        T4 = self.L4(features[3]).transpose(1, 3)  # (B, 8,  16, 224)
        T3 = self.L3(features[2]).transpose(1, 3)  # (B, 8,  32, 224)
        T2 = self.L2(features[1]).transpose(1, 3)  # (B, 8,  64, 224)
        T1 = self.L1(features[0]).transpose(1, 3)  # (B, 8, 128, 224)
...
class RadViT(nn.Module):
    def __init__(self, D, p, H, W, neuron, mha, layer, dropout, n_encoders=1):
        super().__init__()
    ...
    def forward(self, x, x_mask=None):
        ...
        # x = self.pe(x.flatten(2).transpose(1, 2))  # (B, Np, D) where Np=Hp*Wp
            # ---- Encoder ----
        ...
        y = x.view(B, self.D, self.Hp, self.Wp) # [4, 256, 16, 8]

        # ---- Multi-scale features ----
    
        # y: (B, D, 16, 8)
        feat3 = self.feat_3(y)   
        feat2 = self.feat_2(F.interpolate(y, size=(32, 8), mode='bilinear', align_corners=False)) 
        feat1 = self.feat_1(F.interpolate(y, size=(64, 8), mode='bilinear', align_corners=False))
        feat0 = self.feat_0(F.interpolate(y, size=(128, 8), mode='bilinear', align_corners=False))
